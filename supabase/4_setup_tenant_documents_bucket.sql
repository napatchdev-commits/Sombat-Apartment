...
create policy if not exists "tenant-documents allow all select"
  on storage.objects for select
  using (bucket_id = 'tenant-documents');

create policy if not exists "tenant-documents allow all insert"
  on storage.objects for insert
  with check (bucket_id = 'tenant-documents');

create policy if not exists "tenant-documents allow all update"
  on storage.objects for update
  using (bucket_id = 'tenant-documents');

create policy if not exists "tenant-documents allow all delete"
